...
When user asks for code review, PR review, diff review, or change-risk review, load:
@ref[.resources/prompts/code-review.md]
@ref[.resources/skills/code-review.md]

Then load only matching synced Claude skills from `.claude/skills/` for changed scope:
- `code-review-agentic-artifacts` for `SKILL.md`, `*.skill.md`, `*.agent.md`, `*.prompt.md`, `*.instructions.md`, `copilot-instructions.md`, `CLAUDE.md`, `AGENTS.md`, and MCP config files
- `code-review-c`, `code-review-cpp`, `code-review-go`, `code-review-python`, `code-review-rust`, `code-review-javascript`, `code-review-typescript`, `code-review-html`, `code-review-jenkins-groovy`, `code-review-robot` for matching changed files
- Mixed diffs may load multiple language skills. Agentic-artifact review should run alongside language review when routing or instruction files change.

## Shell Safety

NEVER use heredoc (<<EOF) in terminal commands. Use create_file or file writing tools instead. No exceptions.
